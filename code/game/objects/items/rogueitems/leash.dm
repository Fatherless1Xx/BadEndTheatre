...
/obj/item/leash/attack(mob/living/carbon/C, mob/living/user)
	...
	if((collar && collar:leashable == TRUE) || istype(C.get_item_by_slot(ITEM_SLOT_HANDCUFFED), /obj/item/rope/chain))
		...
		if(C.handcuffed)
			leashtime = 5
		if(do_after(user, leashtime, C)) //do_after adds a progress bar, but then we also check to see if they have a collar
...
/proc/apply_tug_mob_to_mob(mob/living/target, mob/living/source, distance = 1)
	if(!target || !source)
		return
	if(get_dist(target, source) <= distance)
		return
	for(var/i in 1 to max(1, get_dist(target, source) - distance))
...
/obj/item/catbell/attack_self(mob/living/user)
	if(world.time < last_ring + 15)
		return
	user.visible_message(span_info("[user] fiddles with the [src]."))
	last_ring = world.time


//Bell attachment onto collars
/obj/item/catbell/attack(mob/living/carbon/target, mob/living/user)
	var/obj/item/clothing/neck/roguetown/collar/leather/collar = target.get_item_by_slot(ITEM_SLOT_NECK)
	if(!istype(collar))
	...
	if(!do_after(user, target.handcuffed ? 0.5 SECONDS : 5 SECONDS, target))
		return
...
// Crafting entries from the source codebase are not wired in this system; omitted for now to avoid compile issues.
